...
#create the cursor - what tells the database to do
...
# insert values into typing table
cursor.execute("""
...
cursor.execute("""
    CREATE TABLE IF NOT EXISTS pokemon_master (
        ...
        weight REAL,
        pokedex_entry TEXT,
        FOREIGN KEY (type1) REFERENCES pokemon_type(type_id),
        FOREIGN KEY (type2) REFERENCES pokemon_type(type_id)   
    )
""")

# insert information into pokemon master table - (all info for pokemon with types set as integers)
cursor.execute("""
               INSERT INTO pokemon_master(master_id, species, type1, type2, feet, inches, weight, pokedex_entry) VALUES
...
# Create pokemon data - (i.e. master pokemon table with types as strings instead of ints)
cursor.execute("""
    CREATE TABLE IF NOT EXISTS pokemon_data AS
    SELECT 
        pm.master_id,					
        pm.species,					
        pt1.typing AS type1,
...
for row in cursor.execute("SELECT * FROM pokemon_data"):
    print(row)
